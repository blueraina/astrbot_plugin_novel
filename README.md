...
![群聊小说模块流程图](docs/images/chat-novel-module-flow.png)

### 小说模块

![小说模块流程图](docs/images/novel-module-flow.png)

### v3.1 更新内容

- **参考 DeepSeek CSA/HCA 思路的长上下文增强**：群聊小说新增 HCA 式故事档案与 CSA 式相关历史记忆召回，生成前自动注入主线、核心冲突、角色状态、伏笔、最近章节和相关历史事件，提升前后文相关度
- **剧情检查器**：章节生成后额外检查主线推进、连续性、冲突和伏笔，检查结果保存到章节数据中
- **正文生成更稳定**：群聊小说改为“正文纯文本生成 + 短 JSON 元数据提取”，避免长正文塞进 JSON 导致括号未闭合、乱码或整章保存失败
- **下一章剧情走向**：新增 `/群聊小说 剧情走向 <内容>`，可临时指定下一次章节生成方向，生成成功后自动清空
- **封面生成增强**：新增 OpenAI Images HTTP API 支持，适配 `gpt-image-2`、第三方 OpenAI Images 网关和 Responses `image_generation` 兼容模式
- **关系图独立配置**：关系图生成模型可通过 `provider_relationship_graph` 单独配置，超时时间可通过 `chat_novel_relationship_timeout` 设置
- **群聊小说 bug 修复**：修复长设定可能被截断、新群友未进入出场人物、正文开头重复出现“第X章/章节标题”等问题

### v3.0 新增特性

- **🎯 质量阈值可配置**：`chat_novel_quality_threshold` 设置有效消息占比阈值（0-100，默认 40），低于此值判定内容不足
- **🧭 主线记忆增强**：群聊小说生成前自动注入 HCA 故事档案、CSA 相关历史记忆和最近章节上下文，生成后进行剧情推进检查
- **🔍 小模型过滤无用消息**：`chat_novel_filter_enabled` 开启后，内容不足时自动用小模型过滤无效消息，仅保留有效消息继续收集
- **📝 自定义设定**：`/群聊小说 设定 <内容>` 中途添加设定，导出时显示在简介中
- **🧭 下一章剧情走向**：`/群聊小说 剧情走向 <内容>` 指定下一次生成的剧情方向，生成成功后自动清空
...
| `chat_novel_memory_enabled` | bool | true | 群聊小说：启用 HCA 故事档案 + CSA 相关记忆召回 |
| `chat_novel_memory_top_k` | int | 8 | 群聊小说：每章生成前召回的相关历史记忆条数 |
| `chat_novel_plot_check_enabled` | bool | true | 群聊小说：生成后额外检查主线推进、连续性、冲突和伏笔 |
| `chat_novel_relationship_timeout` | int | 120 | 群聊小说：关系图生成超时（秒） |
...
| `provider_relationship_graph` | 关系图生成 AI | 群聊小说关系图 |
...
| `cover_image_model` | string | `""` | 绘图模型名称（如 `gpt-image-2`、`seedream-3.0`、`dall-e-3`，留空由 API 自动选择） |
| `cover_image_api_key` | string | `""` | OpenAI Images API Key（可选；填写后直接调用 `/v1/images/generations` 或 `/v1/images/edits`，适用于 `gpt-image-2`） |
| `cover_image_base_url` | string | `""` | OpenAI Images API Base URL（可选；默认 `https://api.openai.com/v1`，兼容第三方 Images 网关） |
| `cover_image_responses_model` | string | `""` | Responses `image_generation` 兼容模式使用的主模型（可选；第三方网关报 `image_generation tools` 错误时可填 `gpt-4o` 或 `gpt-5`） |
...
| 关系图生成 | `provider_relationship_graph` | 轻量/结构化输出稳定的模型 |
...
临时指定**下一次生成**必须遵循的剧情方向，生成成功后会自动清空：

```
/群聊小说 剧情走向 下一章让主角团发现内奸，并在结尾留下反转
/群聊小说 剧情走向 清空
```
...
- 关系图分析模型可在 WebUI 中通过 `provider_relationship_graph` 单独配置；LLM 生成超时由 `chat_novel_relationship_timeout` 控制
...
| 下一章剧情走向 | 用户通过 `/群聊小说 剧情走向` 设置，仅下一次生成有效 |
...
| `chat_novel_memory_enabled` | bool | true | 启用 HCA 故事档案 + CSA 相关记忆召回 |
| `chat_novel_memory_top_k` | int | 8 | 每章生成前召回的相关历史记忆条数 |
| `chat_novel_plot_check_enabled` | bool | true | 章节生成后是否进行剧情推进/连续性/冲突/伏笔检查 |
| `chat_novel_relationship_timeout` | int | 120 | 关系图生成 AI 调用超时（秒） |
...
| `/群聊小说 剧情走向 <内容>` | 指定下一次生成章节的剧情方向 | **v3.1** |
