...
- **🔇 群聊小说预览控制**：
  - 新增配置项 `chat_novel_preview_limit`：设置生成章节后发送的预览文本字数上限（默认 800 字，0 = 不限制）
  - 新增指令：`/群聊小说 关闭预览` 和 `/群聊小说 开启预览`，可在群聊中开关章节预览文本
...
| `chat_novel_preview_limit` | int | 800 | 群聊小说：生成章节后发送的预览文本字数上限（0 = 不限制） |
