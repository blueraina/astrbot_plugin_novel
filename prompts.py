...
## 本次用户指定剧情走向（仅本章有效，必须优先执行）
{next_plot_direction}

## HCA 故事档案（高压缩全局记忆，必须优先遵守）
{story_bible}
...
## 本章素材 — 群聊天记录
以下是群聊天的原始记录。为了防止冒名顶替，发言者名字后附带了 (ID:真实QQ号)。
请以此为灵感素材，将群友们的对话、互动、话题转化为小说情节。
注意：不要直接照搬聊天内容！**绝对不要在小说正文中写出QQ号或ID**！请直接使用他们的小说角色名进行创作。
...
3. 保持与 HCA 故事档案、近邻上下文、CSA 相关记忆的连贯性
4. 包含充分的环境描写、人物心理活动、对话互动和动作细节
5. 对话要生动自然、符合角色性格
6. 适当加入感官描写增强沉浸感
7. 故事情节要有起承转合，不要流水账
8. 将群友的话题和互动巧妙融入情节
9. 本章必须至少推进一条主线、冲突、关系变化或伏笔；如果当前素材很散，请主动围绕主线筛选素材
10. 如果故事档案中的主线/核心冲突尚未明确，请在本章建立一个清晰但可持续发展的主线方向
11. 正文开头不要写“第X章”“第一章标题”“章节标题”等标题行，直接从小说正文第一段开始
{ending_instruction}

## 输出格式
只输出章节正文纯文本。
不要输出 JSON，不要使用 Markdown 代码块，不要解释你的写作思路，不要在正文中写出 QQ 号或 ID。
"""


CHAT_NOVEL_EXTRACT_CHAPTER_METADATA_PROMPT = """\
你是一位小说编辑。请根据新生成的章节正文，提取用于后续章节连续创作的短结构化信息。
...
请只返回短 JSON，不要包含完整正文 content 字段：
```json
{{
  ...
  "story_bible": {{
    "mainline": "当前小说主线目标（一句话，明确可推进）",
    "current_arc": "当前篇章阶段/阶段目标",
    "core_conflict": "当前最重要的矛盾或阻力",
    "narrative_tone": "叙事基调",
    "world_state": "世界/局势当前状态",
    "character_states": ["重要角色当前状态，每条不超过40字"],
    "unresolved_hooks": ["尚未解决的伏笔或悬念"],
    "resolved_hooks": ["本章解决的伏笔或阶段性结论"],
    "important_facts": ["后续必须记住的事实"],
    "next_direction": "下一章最自然的推进方向"
  }},
  "memory_entries": [
    {{
      "type": "event/character/hook/conflict/world/relationship",
      "title": "记忆标题",
      "content": "后续召回时有用的具体事实，避免空泛总结",
      "characters": ["相关角色名"],
      "keywords": ["关键词"],
      "importance": 3
    }}
...
你是一位严谨的小说剧情审校。请检查新生成的群聊小说章节是否真正推进了故事，并指出后续需要注意的问题。
...
## 检查重点
1. 是否推进主线或当前篇章目标
2. 是否与近邻上下文和召回的历史记忆保持连续
3. 是否有明确冲突、阻力、选择、代价或转折
4. 是否埋下或回应伏笔
5. 是否避免把群聊素材机械堆砌成流水账

请只返回 JSON；所有 score 字段为 0-10 的整数：
```json
{{
  "passed": true,
  "mainline_progress_score": 8,
  "continuity_score": 8,
  "conflict_score": 8,
  "hook_score": 8,
  "issues": ["发现的问题"],
...
# 群聊小说 — 图片识别
# =====================================================================
CHAT_NOVEL_IMAGE_RECOGNITION_PROMPT = """\
你是一位极其细心的图片分析专家。请仔细识别以下图片中的所有内容，尽可能详尽地描述。

## 识别要求

1. **尽可能保存所有信息**：不要遗漏任何细节，包括文字、数字、符号、颜色、位置关系等
2. **文字类内容**：如果图片中包含文字、数学公式、代码、表格等文本信息，**必须**将其完整以 LaTeX 格式转录保存。
   - 数学公式用 `$...$` 或 `$$...$$` 包裹
   - 代码用 `\\texttt{{...}}` 表示
   - 表格用 LaTeX tabular 格式
   - 普通文字直接保留原文
3. **非文字内容**：对场景、人物、物品、表情、动作等进行详细描述，包括：
   - 人物外貌、表情、姿态、服装
   - 场景环境、光线、氛围
   - 物品的形状、颜色、材质
   - 构图方式、画面风格
4. **表情包/梗图**：描述图片的核心含义和情绪表达
5. **截图**：完整转录截图中的所有文字内容

## 输出格式
请直接输出识别结果，格式如下：

【图片类型】（如：照片/截图/表情包/插画/公式/图表等）
【内容描述】详细描述...
【文字内容】（如有）LaTeX 格式的文字转录...
"""
